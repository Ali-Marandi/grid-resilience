...
**Grid Resilience Studio v1.2.0** یک نرم‌افزار دسکتاپ آفلاین برای غربال‌گری مهندسی تاب‌آوری شبکه‌های قدرت است. این نسخه تحلیل DC N-1 و پخش بار AC متعادل را با پایداری گذرای چندماشینهٔ کاهش‌یافته، غربال‌گری N-2 و آبشار بارگذاری، گزارش‌های مهندسی HTML/PDF، رابط Windows با پنل‌های جداشدنی، و زنجیرهٔ انتشار قابل‌راستی‌آزمایی تکمیل می‌کند.
...
| پایداری و امنیت | شبیه‌سازی swing equation چندماشینه، سناریوی fault/clearing، منحنی زاویه و سرعت، CCT screening، N-2 و cascade ranking | پیاده‌سازی‌شده؛ **غربال‌گری کاهش‌یافته است** |
| گزارش‌دهی | خروجی HTML/PDF با منشأ مدل، timestamp و هشدار محدودیت مهندسی | پیاده‌سازی‌شده |
| عرضهٔ ویندوز | ساخت EXE، checksum، JSON وضعیت امضا، GitHub provenance attestation و مسیر اختیاری Authenticode با secrets محافظت‌شده | پیاده‌سازی‌شده؛ امضای واقعی نیازمند گواهی سازمانی است |
...
## پایداری گذرا، N-2 و گزارش‌های مهندسی

v1.2.0 برای مطالعات غربال‌گری، مدل multi-machine swing equation با رویداد fault/clearing و جست‌وجوی bound برای critical clearing time ارائه می‌دهد. تحلیل N-2 جفت خروج خطوط را رتبه‌بندی و overload-cascade را برای آشکارسازی زودهنگام ریسک بررسی می‌کند. رابط Windows نمودارهای swing را نمایش می‌دهد و پنل‌های topology و transient را در پنجره‌های جداشدنی قابل‌همگام باز می‌کند. گزارش‌های HTML/PDF منشأ مدل، زمان تولید و نتایج قابل‌ردیابی را همراه با محدودیت‌ها ثبت می‌کنند.

> خروجی‌های پایداری گذرا و N-2 در این نسخه، **screening engineering** هستند؛ مدل دینامیکی کامل RMS/EMT، کنترل تحریک/گاورنر، حفاظت، پایداری ولتاژ یا اعتبارسنجی عملیاتی نیستند. هیچ setpoint یا پیشنهاد اصلاحی نباید بدون مطالعهٔ تأییدشده و بازبینی مهندس صلاحیت‌دار اجرا شود.
...
خروجی در `dist/GridResilienceStudio.exe` قرار می‌گیرد. GitHub Actions آزمون‌ها را روی Python 3.10 تا 3.13 اجرا می‌کند، EXE را روی runner ویندوز می‌سازد، checksum SHA-256 و وضعیت امضا را تولید می‌کند، provenance attestation ایجاد می‌کند و فقط برای تگ‌های `v*` آن‌ها را به Release ضمیمه می‌کند. راهنمای تنظیم Authenticode و Azure Artifact Signing در [`docs/CODE_SIGNING_GUIDE.md`](docs/CODE_SIGNING_GUIDE.md) قرار دارد.
